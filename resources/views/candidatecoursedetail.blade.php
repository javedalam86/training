                ...
                @if(!empty($latest))
                  @foreach ($latest as $objs)
                    ...
                    <tr>
                      ...
                      <td><b>{{$cqResult->quiz_result}}</b> <a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-sm" onclick=showevaluationmodal({{$obj->candidate_quize_id}})><i class="flaticon2-search"></i></a></td>
                    </tr>
                    @php
                      //break;
                    @endphp
